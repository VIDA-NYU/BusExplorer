...
def readFile(fileName, numLines, dbName, collectionName, erase, skipHeader, useDefaultFormat):
    ...
    for line in f:
        ...
        dateObj = datetime.strptime(tokens[0][0:-1], '%Y-%m-%dT%H:%M:%S') # ok
        hour = dateObj.hour
        ...
        bearing = float(tokens[4]) # ok
        loc = [float(tokens[3]),float(tokens[2])] # ok
...
if __name__ == "__main__":
    ...
    readFile(args.fileName, args.numLines, args.dbName, args.collectionName, args.erase, args.skipHeader)